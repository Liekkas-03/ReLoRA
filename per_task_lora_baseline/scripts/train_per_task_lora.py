...
from src.data import CausalLMCollator, get_train_eval_raw, preprocess_causal_lm_dataset
...
def train_one_task(cfg, task_name: str) -> None:
    ...
    train_raw, eval_raw = get_train_eval_raw(task, cfg.data, cache_dir=cfg.dataset_cache_dir)
    train_ds = preprocess_causal_lm_dataset(train_raw, task, tokenizer, cfg.data)
    eval_ds = preprocess_causal_lm_dataset(eval_raw, task, tokenizer, cfg.data)
    ...
    tokenizer.save_pretrained(str(task_output_dir / "tokenizer"))
    save_json(
        adapter_dir / "task_info.json",
        {
            ...
            "hf_name": task.hf_name,
            "note": "Independent per-task LoRA adapter trained from the original base model.",
        },
    )

    metrics = trainer.evaluate()
    save_json(task_output_dir / "eval_metrics.json", metrics)
